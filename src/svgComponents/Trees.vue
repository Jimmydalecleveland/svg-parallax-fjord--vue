...
function blowTree(treePaths, animationTime) {
  if (!treePaths || !treePaths.darkPaths.fromPath) return

  const { darkPaths, lightPaths } = treePaths
  animateElement(darkPaths.target, darkPaths.toPath, animationTime, easeOutBack)
  animateElement(
    lightPaths.target,
    lightPaths.toPath,
    animationTime,
  ...
  function unblowTree() {
    ...
    animateElement(lightPaths.target, lightPaths.fromPath, 2000, easeInOutBack)
  }
}
...
export default {
  ...
  mounted() {
    ...
    function startBlowTrees() {
      blowTrees()
      this.intervalId = setInterval(blowTrees, 8000)
    }
